...
from instance_selection_encapsulation.operator.metrics import fitness_gmean_mauc, fitness_accuracy, \
    evaluate_individuals, fitness_accuracy2
...
def init_toolbox_emosaic(model, x_train, y_train, n_splits=5, random_seed=42):
    NDIM = len(y_train)  # 个体基因长度

    creator.create("FitnessMaxAndMax", base.Fitness, weights=(1.0, 1.0))  # 最大化评价目标
    ...
    toolbox.register("evaluate", evaluate_individuals, model=model, x_train=x_train, y_train=y_train, n_splits=n_splits,
                     random_seed=random_seed, fitness_function=fitness_gmean_mauc)  # 评价个体
...
def init_toolbox_eseic(model, x_train, y_train, weights_train, constraints, n_splits=5, random_seed=42):
    NDIM = len(y_train)  # 个体基因长度
    ...
    toolbox.register("init_population", init_population_based_balanced_method, y_train=y_train,
                     ratio=0.9)  # 初始化为平衡数据集（实例个数为min*0.9）
    toolbox.register("population", tools.initRepeat, list, toolbox.individual)  # 种群初始化
    toolbox.register("fitness_function", fitness_accuracy2, weights_train=weights_train)
    toolbox.register("evaluate", evaluate_individuals, model=model, x_train=x_train, y_train=y_train, n_splits=n_splits,
...
def init_toolbox_eseic2(model, x_train, y_train, weights_train, constraints, n_splits=5, random_seed=42):# 两个目标
    NDIM = len(y_train)  # 个体基因长度
    creator.create("FitnessMaxAndMax", base.Fitness, weights=(1.0, 1.0))  # 最大化评价目标
    creator.create("Individual", array.array, typecode='i', fitness=creator.FitnessMaxAndMax, pfc=None, model=None,
                   y_sub_and_pred_proba=None, gmean=None, mauc=None)
